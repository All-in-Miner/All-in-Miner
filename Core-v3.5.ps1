...
Function InitApplication {
    . .\include.ps1
	Update-Status("*** All-in-Miner Core v3.5 ***")
...
Function NPMCycle {
    ...
    Update-Status("Getting miner information...")
    ...
    # select only the miners that match $Config.MinerName, if specified, and don't match $Config.ExcludeMiners
    $Variables.Miners = if (Test-Path "Miners") {Get-ChildItemContent "Miners" | ForEach {$_.Content | Add-Member @{Name = $_.Name} -PassThru} | 
            ...
            Where {$Config.ExcludeMiners.Count -eq 0 -or (Compare $Config.ExcludeMiners $_.Name -IncludeEqual -ExcludeDifferent | Measure).Count -eq 0}
    }
